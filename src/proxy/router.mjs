...
const CONFIG = {
	...
	simpleKeywordsExact: ['hi', 'ok', 'okay', 'hey', 'yo', 'sup', 'bye', 'thx'],
	agenticKeywords: [
		'read file',
		'read the file',
		'look at',
...
function scoreMultiStep(text) {
	...
	if (hits.length >= 2)
		return { name: 'multiStepPatterns', score: 0.7, signal: 'multi-step (strong)' }
...
function scoreSimple(text) {
	...
	if (total >= 2)
		return { name: 'simpleIndicators', score: -1.0, signal: `simple (${examples.join(', ')})` }
	if (total >= 1)
		return { name: 'simpleIndicators', score: -0.8, signal: `simple (${examples.join(', ')})` }
...
export function classify(prompt, systemPrompt) {
	...
	const dimensions = [
		...
		scoreKeywords(
			text,
			CONFIG.agenticKeywords,
			'agenticTask',
			'agentic',
			{ low: 2, high: 4 },
			{ none: 0, low: 0.3, high: 0.6 },
		),
		scoreKeywords(
			text,
			CONFIG.constraintKeywords,
			'constraintCount',
			'constraints',
			{ low: 1, high: 3 },
			{ none: 0, low: 0.3, high: 0.6 },
...
async function main() {
	...
	if (jsonFlag) {
		...
		if (decision.signals.length > 0) console.log(`  Signals:    ${decision.signals.join(' | ')}`)
		console.log(bar)
	}
}
