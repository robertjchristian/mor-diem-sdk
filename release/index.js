...
var isLE, swap32IfBE, hasHexBuiltin, hexes, asciis
...
function weierstrass(curveDef) {
	...
	const {
		...
		fromBytes(bytes) {
			...
			if (len === compressedLen && (head === 2 || head === 3)) {
				...
				if (!inRange(x, _1n4, Fp.ORDER)) throw new Error('Point is not on curve')
	...
	const bits2int_modN = CURVE.bits2int_modN || ((bytes) => modN(bits2int(bytes)))
...
function SWUFpSqrtRatio(Fp, Z) {
	const q = Fp.ORDER
	let l = _0n4
	for (let o = q - _1n4; o % _2n2 === _0n4; o /= _2n2) l += _1n4
	const c1 = l
...
function schnorrVerify(signature, message, publicKey) {
	...
	try {
		...
		if (!inRange(r, _1n5, secp256k1P)) return false
		const s = num(sig.subarray(32, 64))
		if (!inRange(s, _1n5, secp256k1N)) return false
			...
			const details =
				args.cause instanceof BaseError
					...
					: args.cause?.message
						...
						: args.details
			const docsPath =
				args.cause instanceof BaseError ? args.cause.docsPath || args.docsPath : args.docsPath
			const message = [
...
var init_signatures = __esm(() => {
	...
	modifiers = new Set(['memory', 'indexed', 'storage', 'calldata'])
	eventModifiers = new Set(['indexed'])
	functionModifiers = new Set(['calldata', 'memory', 'storage'])
})
...
var init_cache = __esm(() => {
	parameterCache = new Map([
		...
		['event:address indexed from', { type: 'address', name: 'from', indexed: true }],
		['event:address indexed to', { type: 'address', name: 'to', indexed: true }],
		['event:uint indexed tokenId', { type: 'uint256', name: 'tokenId', indexed: true }],
		['event:uint256 indexed tokenId', { type: 'uint256', name: 'tokenId', indexed: true }],
	])
})
...
function parseConstructorSignature(signature, structs = {}) {
	...
	if (!match) throw new InvalidSignatureError({ signature, type: 'constructor' })
	const params = splitParameters(match.parameters)
	const abiParameters = []
	const length = params.length
	for (let i = 0; i < length; i++)
...
`),
	...
	AbiEncodingArrayLengthMismatchError = class AbiEncodingArrayLengthMismatchError extends (
		BaseError2
	) {
...
`),
	...
	AbiEncodingBytesSizeMismatchError = class AbiEncodingBytesSizeMismatchError extends BaseError2 {
		constructor({ expectedSize, value }) {
			super(
				`Size of bytes "${value}" (bytes${size(value)}) does not match expected size (bytes${expectedSize}).`,
...
`),
	...
	DecodeLogDataMismatch = class DecodeLogDataMismatch extends BaseError2 {
		constructor({ abiItem, data, params, size: size2 }) {
			super(
				[`Data size of ${size2} bytes is too small for non-indexed event parameters.`].join(`
`),
				{
					metaMessages: [
						`Params: (${formatAbiParams(params, { includeName: true })})`,
...
`),
	...
	InvalidAbiEncodingTypeError = class InvalidAbiEncodingTypeError extends BaseError2 {
		constructor(type, { docsPath }) {
			super(
				[`Type "${type}" is not a valid encoding type.`, 'Please provide a valid ABI type.'].join(`
`),
	...
	InvalidAbiDecodingTypeError = class InvalidAbiDecodingTypeError extends BaseError2 {
		constructor(type, { docsPath }) {
			super(
				[`Type "${type}" is not a valid decoding type.`, 'Please provide a valid ABI type.'].join(`
`),
	...
	InvalidArrayError = class InvalidArrayError extends BaseError2 {
		constructor(value) {
			super(
				[`Value "${value}" is not a valid array.`].join(`
`),
				{
...
var init_data = __esm(() => {
	...
	InvalidBytesLengthError = class InvalidBytesLengthError extends BaseError2 {
		constructor({ size: size2, targetSize, type }) {
			super(
				`${type.charAt(0).toUpperCase()}${type.slice(1).toLowerCase()} is expected to be ${targetSize} ${type} long, but is ${size2} ${type} long.`,
...
var init_encoding = __esm(() => {
	...
	InvalidHexBooleanError = class InvalidHexBooleanError extends BaseError2 {
		constructor(hex2) {
			super(
				`Hex value "${hex2}" is not a valid boolean. The hex value must be "0x0" (false) or "0x1" (true).`,
...
function hexToBigInt(hex2, opts = {}) {
	const { signed } = opts
	if (opts.size) assertSize(hex2, { size: opts.size })
...
function hexToBool(hex_, opts = {}) {
	...
	if (trim(hex2) === '0x00') return false
	if (trim(hex2) === '0x01') return true
...
function numberToHex(value_, opts = {}) {
	...
	if (size2) return pad(hex2, { size: size2 })
	return hex2
}
...
function toBytes2(value, opts = {}) {
	...
	if (isHex(value)) return hexToBytes3(value, opts)
...
var init_sha3 = __esm(() => {
	...
	for (let round = 0, R = _1n6, x = 1, y = 0; round < 24; round++) {
		...
		for (let j = 0; j < 7; j++) {
			...
			if (R & _2n4) t ^= _1n6 << ((_1n6 << /* @__PURE__ */ BigInt(j)) - _1n6)
		}
		_SHA3_IOTA.push(t)
	}
...
function normalizeSignature(signature) {
	...
	for (let i = 0; i < signature.length; i++) {
		...
		if (!active) continue
		if (level === 0) {
			if (char === ' ' && ['event', 'function', ''].includes(result)) result = ''
			else {
				result += char
				if (char === ')') {
					valid = true
...
function prepareEncodeFunctionData(parameters) {
	...
	if (functionName) {
		...
		if (!item) throw new AbiFunctionNotFoundError(functionName, { docsPath: docsPath2 })
		abiItem = item
	}
	if (abiItem.type !== 'function')
...
function bytesToBigInt(bytes, opts = {}) {
	...
	const hex2 = bytesToHex3(bytes, opts)
...
function decodeBytes(cursor, param, { staticPosition }) {
	...
	const value = bytesToHex3(cursor.readBytes(Number.parseInt(size2, 10), 32))
	return [value, 32]
}
function decodeNumber(cursor, param) {
	...
	const value = cursor.readBytes(32)
	return [size2 > 48 ? bytesToBigInt(value, { signed }) : bytesToNumber2(value, { signed }), 32]
}
...
function hasDynamicChild(param) {
	...
	const arrayComponents = getArrayComponents(param.type)
	if (arrayComponents && hasDynamicChild({ ...param, type: arrayComponents[1] })) return true
	return false
}
var sizeOfLength = 32,
	sizeOfOffset = 32
...
function decodeErrorResult(parameters) {
	...
	const abiItem = abi_.find(
		(x) => x.type === 'error' && signature === toFunctionSelector(formatAbiItem2(x)),
	)
	if (!abiItem)
...
var init_transaction = __esm(() => {
	...
	InvalidSerializableTransactionError = class InvalidSerializableTransactionError extends (
		BaseError2
	) {
		...
		constructor(
			...
			const prettyArgs = prettyPrint({
				...
				value:
					...
					`${formatEther(value)} ${chain2?.nativeCurrency?.symbol || 'ETH'}`,
				data,
				gas,
				gasPrice: typeof gasPrice !== 'undefined' && `${formatGwei(gasPrice)} gwei`,
				maxFeePerGas: typeof maxFeePerGas !== 'undefined' && `${formatGwei(maxFeePerGas)} gwei`,
				maxPriorityFeePerGas:
					typeof maxPriorityFeePerGas !== 'undefined' && `${formatGwei(maxPriorityFeePerGas)} gwei`,
				nonce,
			})
	...
	WaitForTransactionReceiptTimeoutError = class WaitForTransactionReceiptTimeoutError extends (
		BaseError2
	) {
		constructor({ hash: hash2 }) {
		...
		constructor(cause, { abi, args, contractAddress, docsPath: docsPath3, functionName, sender }) {
			...
			const prettyArgs = prettyPrint({
				...
				args:
					formattedArgs &&
					formattedArgs !== '()' &&
					`${[...Array(functionName?.length ?? 0).keys()].map(() => ' ').join('')}${formattedArgs}`,
				sender,
			})
			super(
				cause.shortMessage ||
					...
					if (errorName === 'Error') {
						...
						metaMessages = [
							...
							formattedArgs && formattedArgs !== '()'
								? `       ${[...Array(errorName?.length ?? 0).keys()].map(() => ' ').join('')}${formattedArgs}`
								: '',
						]
					}
				} catch (err) {
...
`)
	...
	CounterfactualDeploymentFailedError = class CounterfactualDeploymentFailedError extends (
		BaseError2
	) {
		constructor({ factory }) {
			super(
				`Deployment for counterfactual contract call failed${factory ? ` for factory "${factory}".` : ''}`,
				{
					metaMessages: [
						'Please ensure:',
...
`),
	...
	UnsupportedNonOptionalCapabilityError = class UnsupportedNonOptionalCapabilityError extends (
		ProviderRpcError
	) {
		constructor(cause) {
			super(cause, {
	...
	AtomicReadyWalletRejectedUpgradeError = class AtomicReadyWalletRejectedUpgradeError extends (
		ProviderRpcError
	) {
		constructor(cause) {
			super(cause, {
	...
	WalletConnectSessionSettlementError = class WalletConnectSessionSettlementError extends (
		ProviderRpcError
	) {
		constructor(cause) {
			super(cause, {
...
function getNodeError(err, args) {
	...
	const executionRevertedError =
		err instanceof BaseError2 ? err.walk((e) => e?.code === ExecutionRevertedError.code) : err
...
function decodeFunctionResult(parameters) {
	...
	if (functionName) {
		...
		if (!item) throw new AbiFunctionNotFoundError(functionName, { docsPath: docsPath4 })
		abiItem = item
	}
	if (abiItem.type !== 'function')
...
function walk2(err, fn) {
	if (fn?.(err)) return err
	if (err && typeof err === 'object' && 'cause' in err && err.cause) return walk2(err.cause, fn)
	return fn ? null : err
}
var BaseError3
var init_Errors = __esm(() => {
	...
	BaseError3 = class BaseError3 extends Error {
		...
		constructor(shortMessage, options = {}) {
			...
			const docsPath5 = (() => {
				if (options.cause instanceof BaseError3) return options.cause.docsPath || options.docsPath
...
function charCodeToBase162(char) {
	if (char >= charCodeMap2.zero && char <= charCodeMap2.nine) return char - charCodeMap2.zero
	if (char >= charCodeMap2.A && char <= charCodeMap2.F) return char - (charCodeMap2.A - 10)
	if (char >= charCodeMap2.a && char <= charCodeMap2.f) return char - (charCodeMap2.a - 10)
	return
}
function pad2(bytes, options = {}) {
	const { dir, size: size3 = 32 } = options
	if (size3 === 0) return bytes
...
function pad3(hex_, options = {}) {
	...
	if (size4 === 0) return hex_
	const hex2 = hex_.replace('0x', '')
	if (hex2.length > size4 * 2)
		throw new SizeExceedsPaddingSizeError3({
			size: Math.ceil(hex2.length / 2),
			targetSize: size4,
			type: 'Hex',
		})
	return `0x${hex2[dir === 'right' ? 'padEnd' : 'padStart'](size4 * 2, '0')}`
}
...
function stringify2(value, replacer, space) {
	return JSON.stringify(
		...
		(key, value2) => {
			...
			if (typeof value2 === 'bigint') return value2.toString() + bigIntSuffix
			return value2
		},
		space,
	)
...
function slice2(value, start, end, options = {}) {
	...
	if (strict) assertEndOffset2(value_, start, end)
	return value_
}
function toBigInt2(bytes, options = {}) {
	const { size: size4 } = options
	if (typeof size4 !== 'undefined') assertSize2(bytes, size4)
...
function toBoolean(bytes, options = {}) {
	...
	if (bytes_.length > 1 || bytes_[0] > 1) throw new InvalidBytesBooleanError2(bytes_)
...
var init_Bytes = __esm(() => {
	...
	InvalidBytesTypeError = class InvalidBytesTypeError extends BaseError3 {
		constructor(value) {
			super(
				`Value \`${typeof value === 'object' ? stringify2(value) : value}\` of type \`${typeof value}\` is an invalid Bytes value.`,
				{
...
function fromBytes(value, options = {}) {
	let string = ''
	for (let i = 0; i < value.length; i++) string += hexes4[value[i]]
	const hex2 = `0x${string}`
	if (typeof options.size === 'number') {
		assertSize3(hex2, options.size)
...
function fromNumber(value, options = {}) {
	...
	const stringValue = (
		signed && value_ < 0 ? BigInt.asUintN(size4 * 8, BigInt(value_)) : value_
	).toString(16)
	const hex2 = `0x${stringValue}`
	if (size4) return padLeft(hex2, size4)
	return hex2
}
...
function slice3(value, start, end, options = {}) {
	...
	if (strict) assertEndOffset3(value_, start, end)
	return value_
}
...
function toBigInt(hex2, options = {}) {
	...
	if (options.size) assertSize3(hex2, options.size)
	const value = BigInt(hex2)
	if (!signed) return value
	const size4 = (hex2.length - 2) / 2
...
function toNumber(hex2, options = {}) {
	const { signed, size: size4 } = options
	if (!signed && !size4) return Number(hex2)
...
var init_Hex = __esm(() => {
	...
	IntegerOutOfRangeError2 = class IntegerOutOfRangeError2 extends BaseError3 {
		constructor({ max, min, signed, size: size4, value }) {
			super(
				`Number \`${value}\` is not in safe${size4 ? ` ${size4 * 8}-bit` : ''}${signed ? ' signed' : ' unsigned'} integer range ${max ? `(\`${min}\` to \`${max}\`)` : `(above \`${min}\`)`}`,
			)
			Object.defineProperty(this, 'name', {
				enumerable: true,
				configurable: true,
	...
	InvalidHexTypeError = class InvalidHexTypeError extends BaseError3 {
		constructor(value) {
			super(
				`Value \`${typeof value === 'object' ? stringify2(value) : value}\` of type \`${typeof value}\` is an invalid hex type.`,
				{
...
var init_chain = __esm(() => {
	...
	ChainMismatchError = class ChainMismatchError extends BaseError2 {
		constructor({ chain: chain2, currentChainId }) {
			super(
				`The current chain of the wallet (id: ${currentChainId}) does not match the target chain for the transaction (id: ${chain2.id} – ${chain2.name}).`,
				{
					metaMessages: [
...
`),
	...
	InvalidChainIdError = class InvalidChainIdError extends BaseError2 {
		constructor({ chainId }) {
			super(
				typeof chainId === 'number' ? `Chain ID "${chainId}" is invalid.` : 'Chain ID is invalid.',
...
function encodeDeployData(parameters) {
	...
	if (!description) throw new AbiConstructorNotFoundError({ docsPath: docsPath5 })
...
function getCallError(err, { docsPath: docsPath6, ...args }) {
	const cause = (() => {
		...
		if (cause2 instanceof UnknownNodeError) return err
		return cause2
	})()
...
var init_ccip = __esm(() => {
	...
	OffchainLookupResponseMalformedError = class OffchainLookupResponseMalformedError extends (
		BaseError2
	) {
...
function decodeFunctionData(parameters) {
	...
	const description = abi.find(
		(x) => x.type === 'function' && signature === toFunctionSelector(formatAbiItem2(x)),
	)
	if (!description)
...
function encodeErrorResult(parameters) {
	...
	if (errorName) {
		...
		if (!item) throw new AbiErrorNotFoundError(errorName, { docsPath: docsPath6 })
		abiItem = item
	}
	if (abiItem.type !== 'error') throw new AbiErrorNotFoundError(undefined, { docsPath: docsPath6 })
...
function encodeFunctionResult(parameters) {
	...
	if (functionName) {
		...
		if (!item) throw new AbiFunctionNotFoundError(functionName, { docsPath: docsPath7 })
		abiItem = item
	}
	if (abiItem.type !== 'function')
...
async function offchainLookup(client, { blockNumber, blockTag, data, to }) {
	...
	const ccipRequest_ =
		ccipRead && typeof ccipRead?.request === 'function' ? ccipRead.request : ccipRequest
	try {
		if (!isAddressEqual(to, sender)) throw new OffchainLookupSenderMismatchError({ sender, to })
...
async function call(client, args) {
	...
	try {
		...
		const { offchainLookup: offchainLookup2, offchainLookupSignature: offchainLookupSignature2 } =
			await Promise.resolve().then(() => (init_ccip2(), exports_ccip))
...
function getRevertErrorData(err) {
	if (!(err instanceof BaseError2)) return
...
var require_cryptoNode = __commonJS((exports) => {
	...
	exports.crypto =
		nc2 && typeof nc2 === 'object' && 'webcrypto' in nc2
			? nc2.webcrypto
			: nc2 && typeof nc2 === 'object' && 'randomBytes' in nc2
				? nc2
				: undefined
})
...
var require_utils = __commonJS((exports) => {
	...
	var hasHexBuiltin4 = /* @__PURE__ */ (() =>
		typeof Uint8Array.from([]).toHex === 'function' && typeof Uint8Array.fromHex === 'function')()
	var hexes5 = /* @__PURE__ */ Array.from({ length: 256 }, (_, i) =>
		i.toString(16).padStart(2, '0'),
	)
	function bytesToHex4(bytes) {
		abytes4(bytes)
		if (hasHexBuiltin4) return bytes.toHex()
		let hex2 = ''
		for (let i = 0; i < bytes.length; i++) {
			hex2 += hexes5[bytes[i]]
		}
	...
	function asciiToBase163(ch) {
		if (ch >= asciis3._0 && ch <= asciis3._9) return ch - asciis3._0
		if (ch >= asciis3.A && ch <= asciis3.F) return ch - (asciis3.A - 10)
		if (ch >= asciis3.a && ch <= asciis3.f) return ch - (asciis3.a - 10)
		return
	}
	function hexToBytes4(hex2) {
		if (typeof hex2 !== 'string') throw new Error('hex string expected, got ' + typeof hex2)
		if (hasHexBuiltin4) return Uint8Array.fromHex(hex2)
...
var require_hmac = __commonJS((exports) => {
	...
	class HMAC2 extends utils_ts_1.Hash {
		constructor(hash3, _key) {
			...
			for (let i = 0; i < pad4.length; i++) pad4[i] ^= 54
			this.iHash.update(pad4)
			this.oHash = hash3.create()
			for (let i = 0; i < pad4.length; i++) pad4[i] ^= 54 ^ 92
			this.oHash.update(pad4)
			;(0, utils_ts_1.clean)(pad4)
		}
		update(buf) {
...
var require_pbkdf2 = __commonJS((exports) => {
	...
	function pbkdf2Init2(hash3, _password, _salt, _opts) {
		...
		const PRF = hmac_ts_1.hmac.create(hash3, password)
	...
	async function pbkdf2Async2(hash3, password, salt, opts) {
		const { c, dkLen, asyncTick, DK, PRF, PRFSalt } = pbkdf2Init2(hash3, password, salt, opts)
		let prfW
		const arr = new Uint8Array(4)
		const view = (0, utils_ts_1.createView)(arr)
		const u = new Uint8Array(PRF.outputLen)
...
var require__md = __commonJS((exports) => {
	...
	class HashMD2 extends utils_ts_1.Hash {
		...
		digestInto(out) {
			...
			for (let i = pos; i < blockLen; i++) buffer2[i] = 0
		...
		_cloneInto(to) {
			...
			to.length = length
			to.pos = pos
			if (length % blockLen) to.buffer.set(buffer2)
			return to
		}
		clone() {
			return this._cloneInto()
	...
	exports.SHA384_IV = Uint32Array.from([
		...
		1731405415, 4290775857, 2394180231, 1750603025, 3675008525, 1694076839, 1203062813, 3204075428,
	])
...
var require_sha2 = __commonJS((exports) => {
	...
	class SHA2562 extends _md_ts_1.HashMD {
		constructor(outputLen = 32) {
			super(64, outputLen, 8, false)
		...
		process(view, offset) {
			...
			for (let i = 0; i < 64; i++) {
				const sigma1 =
					(0, utils_ts_1.rotr)(E, 6) ^ (0, utils_ts_1.rotr)(E, 11) ^ (0, utils_ts_1.rotr)(E, 25)
				const T1 = (H + sigma1 + (0, _md_ts_1.Chi)(E, F, G) + SHA256_K2[i] + SHA256_W2[i]) | 0
				const sigma0 =
					(0, utils_ts_1.rotr)(A, 2) ^ (0, utils_ts_1.rotr)(A, 13) ^ (0, utils_ts_1.rotr)(A, 22)
	...
	var T224_IV = /* @__PURE__ */ Uint32Array.from([
		2352822216, 424955298, 1944164710, 2312950998, 502970286, 855612546, 1738396948, 1479516111,
		258812777, 2077511080, 2011393907, 79989058, 1067287976, 1780299464, 286451373, 2446758561,
	])
	var T256_IV = /* @__PURE__ */ Uint32Array.from([
		573645204, 4230739756, 2673172387, 3360449730, 596883563, 1867755857, 2520282905, 1497426621,
		2519219938, 2827943907, 3193839141, 1401305490, 721525244, 746961066, 246885852, 2177182882,
	])
...
var require_lib = __commonJS((exports) => {
	...
	exports.bytes =
		...
		exports.bech32 =
		exports.base58check =
		exports.createBase58check =
		exports.base58xmr =
		exports.base58xrp =
		exports.base58flickr =
		exports.base58 =
		exports.base64urlnopad =
		exports.base64url =
		exports.base64nopad =
		exports.base64 =
		exports.base32crockford =
		exports.base32hexnopad =
		exports.base32hex =
		exports.base32nopad =
		exports.base32 =
		exports.base16 =
	...
	function astrArr2(label, input) {
		if (!isArrayOf2(true, input)) throw new Error(`${label}: array of strings expected`)
	}
	function anumArr2(label, input) {
		if (!isArrayOf2(false, input)) throw new Error(`${label}: array of numbers expected`)
	}
	...
	function convertRadix3(data, from14, to) {
		...
		const digits = Array.from(data, (d) => {
			anumber3(d)
			if (d < 0 || d >= from14) throw new Error(`invalid integer: ${d}`)
			return d
		})
	...
	var radix2carry2 = (from14, to) => from14 + (to - gcd2(from14, to))
	var powers2 = /* @__PURE__ */ (() => {
		const res = []
		for (let i = 0; i < 40; i++) res.push(2 ** i)
		return res
	})()
	function convertRadix22(data, from14, to, padding3) {
		aArr2(data)
		if (from14 <= 0 || from14 > 32) throw new Error(`convertRadix2: wrong from=${from14}`)
		if (to <= 0 || to > 32) throw new Error(`convertRadix2: wrong to=${to}`)
		if (radix2carry2(from14, to) > 32) {
			throw new Error(
				`convertRadix2: carry overflow from=${from14} to=${to} carryBits=${radix2carry2(from14, to)}`,
			)
		}
		let carry = 0
		let pos = 0
		const max = powers2[from14]
		const mask = powers2[to] - 1
		const res = []
		for (const n of data) {
			anumber3(n)
			if (n >= max) throw new Error(`convertRadix2: invalid data word=${n} from=${from14}`)
		...
		if (!padding3 && pos >= from14) throw new Error('Excess padding')
		if (!padding3 && carry > 0) throw new Error(`Non-zero padding: ${carry}`)
	...
	exports.base32crockford = chain2(
		...
		join2(''),
		normalize3((s) => s.toUpperCase().replace(/O/g, '0').replace(/[IL]/g, '1')),
	)
	var hasBase64Builtin2 = /* @__PURE__ */ (() =>
	...
	var createBase58check2 = (sha2564) =>
		chain2(
			checksum4(4, (data) => sha2564(sha2564(data))),
			exports.base58,
		)
	...
	function bechChecksum2(prefix, words, encodingConst = 1) {
		...
		for (let i = 0; i < len; i++) chk = bech32Polymod2(chk) ^ (prefix.charCodeAt(i) & 31)
	...
	function genBech322(encoding) {
		...
		function encode4(prefix, words, limit = 90) {
			...
			if (isBytes4(words)) words = Array.from(words)
	...
	var bytesToString2 = (type, bytes) => {
		...
		if (!isBytes4(bytes)) throw new TypeError('bytesToString() expects Uint8Array')
...
var require_bip39 = __commonJS((exports) => {
	...
	var isJapanese2 = (wordlist2) => wordlist2[0] === 'あいこくしん'
	function nfkd2(str) {
	...
	function getCoder2(wordlist2) {
		...
		return base_1.utils.chain(
			base_1.utils.checksum(1, calcChecksum2),
	...
	function entropyToMnemonic2(entropy, wordlist2) {
		...
		return words.join(isJapanese2(wordlist2) ? '　' : ' ')
	}
	...
	var psalt2 = (passphrase) => nfkd2('mnemonic' + passphrase)
	function mnemonicToSeed(mnemonic, passphrase = '') {
		return (0, pbkdf2_1.pbkdf2Async)(sha2_1.sha512, normalize3(mnemonic).nfkd, psalt2(passphrase), {
			c: 2048,
			dkLen: 64,
		})
	}
	function mnemonicToSeedSync2(mnemonic, passphrase = '') {
		return (0, pbkdf2_1.pbkdf2)(sha2_1.sha512, normalize3(mnemonic).nfkd, psalt2(passphrase), {
			c: 2048,
			dkLen: 64,
		})
	}
...
init__md()
init_utils()
var Rho160 = /* @__PURE__ */ Uint8Array.from([7, 4, 13, 1, 10, 6, 15, 3, 12, 0, 9, 5, 2, 14, 11, 8])
...
var idxLR = /* @__PURE__ */ (() => {
	...
	const R = [Pi160]
	const res = [L, R]
	for (let i = 0; i < 4; i++) for (const j of res) j.push(j[i].map((k) => Rho160[k]))
	return res
})()
...
var Kl160 = /* @__PURE__ */ Uint32Array.from([0, 1518500249, 1859775393, 2400959708, 2840853838])
var Kr160 = /* @__PURE__ */ Uint32Array.from([1352829926, 1548603684, 1836072691, 2053994217, 0])
...
function radix(num2) {
	...
	return {
		encode: (bytes) => {
			if (!isBytes3(bytes)) throw new Error('radix.encode input should be Uint8Array')
...
function radix2(bits, revPadding = false) {
	...
	return {
		encode: (bytes) => {
			if (!isBytes3(bytes)) throw new Error('radix2.encode input should be Uint8Array')
...
function numberToBytes(num2) {
	if (typeof num2 !== 'bigint') throw new Error('bigint expected')
...
function encodeArg({ param, value }) {
	if (param.type === 'string' || param.type === 'bytes') return keccak256(toBytes2(value))
...
function getContractError(err, { abi, address, args, docsPath: docsPath3, functionName, sender }) {
	...
	const cause = (() => {
		...
		if (
			...
			(code === InvalidInputRpcError.code && details === 'execution reverted' && data)
		) {
...
function toRlp(bytes, to = 'hex') {
	...
	if (to === 'hex') return bytesToHex3(cursor.bytes)
	return cursor.bytes
}
...
function hashAuthorization(parameters) {
	...
	const hash2 = keccak256(
		concatHex([
			'0x05',
			toRlp([chainId ? numberToHex(chainId) : '0x', address, nonce ? numberToHex(nonce) : '0x']),
		]),
	)
	if (to === 'bytes') return hexToBytes3(hash2)
	return hash2
}
...
async function internal_estimateMaxPriorityFeePerGas(client, args) {
	...
	try {
		const maxPriorityFeePerGas =
			chain2?.fees?.maxPriorityFeePerGas ?? chain2?.fees?.defaultPriorityFee
...
async function internal_estimateFeesPerGas(client, args) {
	...
	const gasPrice =
		request?.gasPrice ?? multiply(await getAction(client, getGasPrice, 'getGasPrice')({}))
	return {
		gasPrice,
	}
}
...
function blobsToCommitments(parameters) {
	...
	const blobs =
		typeof parameters.blobs[0] === 'string'
			? parameters.blobs.map((x) => hexToBytes3(x))
			: parameters.blobs
	const commitments = []
...
function blobsToProofs(parameters) {
	...
	const commitments =
		typeof parameters.commitments[0] === 'string'
			? parameters.commitments.map((x) => hexToBytes3(x))
...
var maxBytesPerTransaction =
	bytesPerBlob * blobsPerTransaction - 1 - 1 * fieldElementsPerBlob * blobsPerTransaction
...
class InvalidVersionedHashVersionError extends BaseError2 {
	constructor({ hash: hash2, version: version3 }) {
		super(`Versioned hash "${hash2}" version is invalid.`, {
			metaMessages: [`Expected: ${versionedHashVersionKzg}`, `Received: ${version3}`],
...
async function fillTransaction(client, parameters) {
	...
	try {
		...
		const multiplyFee = (base) =>
			(base * BigInt(Math.ceil(feeMultiplier * denominator))) / BigInt(denominator)
...
// node_modules/viem/_esm/actions/wallet/prepareTransactionRequest.js
var defaultParameters = ['blobVersionedHashes', 'chainId', 'fees', 'gas', 'nonce', 'type']
...
async function prepareTransactionRequest(client, args) {
	...
	const attemptFill = (() => {
		if (
			(parameters.includes('blobVersionedHashes') || parameters.includes('sidecars')) &&
			request.kzg &&
			request.blobs
		)
			return false
		...
		if (
			...
			(typeof request.maxFeePerGas !== 'bigint' || typeof request.maxPriorityFeePerGas !== 'bigint')
		)
			return true
	...
	if (parameters.includes('chainId')) request.chainId = await getChainId2()
		...
		return BigInt(
			await client.request({
				...
				params: rpcStateOverride
					? [request, block ?? client.experimental_blockTag ?? 'latest', rpcStateOverride]
					: block
						? [request, block]
						: [request],
			}),
...
function decodeEventLog(parameters) {
	...
	if (!signature) throw new AbiEventSignatureEmptyTopicsError({ docsPath: docsPath3 })
...
function parseEventLogs(parameters) {
	...
	const eventName = (() => {
		if (!parameters.eventName) return
...
async function getLogs(
	client,
	{ address, blockHash, fromBlock, toBlock, event, events: events_, args, strict: strict_ } = {},
) {
...
function observe(observerId, callbacks, fn) {
	...
	for (const key in callbacks) {
		emit[key] = (...args) => {
			...
			for (const listener of listeners2) listener.fns[key]?.(...args)
		}
	}
					...
					const transport = (() => {
						if (client.transport.type === 'fallback') {
							const transport2 = client.transport.transports.find(
								(transport3) =>
									transport3.config.type === 'webSocket' || transport3.config.type === 'ipc',
							)
							if (!transport2) return client.transport
...
function assertCurrentChain({ chain: chain2, currentChainId }) {
	if (!chain2) throw new ChainNotFoundError()
	if (currentChainId !== chain2.id) throw new ChainMismatchError({ chain: chain2, currentChainId })
}
// node_modules/viem/_esm/actions/wallet/sendTransaction.js
init_transactionRequest()
init_lru()
...
var defineTransactionReceipt = /* @__PURE__ */ defineFormatter(
	'transactionReceipt',
	...
	try {
		...
		if (
			...
			(error.name === 'MethodNotFoundRpcError' ||
				...
				error.details.toLowerCase().includes('does not exist / is not available') ||
				error.details.toLowerCase().includes('missing or invalid. request()') ||
				error.details.toLowerCase().includes('did not match any variant of untagged enum') ||
				error.details.toLowerCase().includes('account upgraded to unsupported contract') ||
			...
			return {
				id: concat([...hashes, numberToHex(chain2.id, { size: 32 }), fallbackMagicIdentifier]),
			}
		}
		throw getTransactionError(err, {
			...parameters,
...
async function getCallsStatus(client, parameters) {
	async function getStatus(id) {
		...
		if (isTransactions) {
			...
			const status2 = (() => {
				if (receipts2.some((r) => r === null)) return 100
				if (receipts2.every((r) => r?.status === '0x1')) return 200
				if (receipts2.every((r) => r?.status === '0x0')) return 500
				return 600
			})()
			return {
				atomic: false,
	...
	const [status, statusCode] = (() => {
		...
		if (statusCode2 >= 100 && statusCode2 < 200) return ['pending', statusCode2]
		if (statusCode2 >= 200 && statusCode2 < 300) return ['success', statusCode2]
		if (statusCode2 >= 300 && statusCode2 < 700) return ['failure', statusCode2]
		if (statusCode2 === 'CONFIRMED') return ['success', 200]
		if (statusCode2 === 'PENDING') return ['pending', 100]
		return [undefined, statusCode2]
	})()
	return {
		...response,
		atomic,
		chainId: chainId ? hexToNumber2(chainId) : undefined,
		receipts:
...
function createClient(parameters) {
	...
	const experimental_blockTag =
		...
		(typeof chain2?.experimental_preconfirmationTime === 'number' ? 'pending' : undefined)
	const blockTime = chain2?.blockTime ?? 12000
	...
	function extend(base) {
		return (extendFn) => {
			...
			for (const key2 in client) delete extended[key2]
...
function isNullUniversalResolverError(err) {
	if (!(err instanceof BaseError2)) return false
...
function namehash(name) {
	...
	if (!name) return bytesToHex3(result)
	const labels = name.split('.')
	for (let i = labels.length - 1; i >= 0; i -= 1) {
		const hashFromEncodedLabel = encodedLabelToLabelhash(labels[i])
		const hashed = hashFromEncodedLabel
			? toBytes2(hashFromEncodedLabel)
...
function labelhash(label) {
	...
	if (!label) return bytesToHex3(result)
...
async function parseAvatarRecord(client, { gatewayUrls, record }) {
	if (/eip155:/i.test(record)) return parseNftAvatarUri(client, { gatewayUrls, record })
...
async function parseNftAvatarUri(client, { gatewayUrls, record }) {
	...
	if (
		isOnChain &&
		(resolvedNftUri.includes('data:application/json;base64,') || resolvedNftUri.startsWith('{'))
	) {
...
async function getEnsAvatar(
	client,
	{ blockNumber, blockTag, assetGatewayUrls, name, gatewayUrls, strict, universalResolverAddress },
) {
...
async function getEnsResolver(client, parameters) {
	...
	if (tlds && !tlds.some((tld) => name.endsWith(tld)))
		throw new Error(
			`${name} is not a valid ENS TLD (${tlds?.join(', ')}) for chain "${chain2.name}" (id: ${chain2.id}).`,
		)
...
async function getEip712Domain(client, parameters) {
	...
	try {
		const [fields, name, version4, chainId, verifyingContract, salt, extensions] = await getAction(
			client,
			readContract,
			'readContract',
		)({
...
function assertTransactionEIP4844(transaction) {
	...
	if (blobVersionedHashes) {
		...
		for (const hash2 of blobVersionedHashes) {
			...
			if (size_ !== 32) throw new InvalidVersionedHashSizeError({ hash: hash2, size: size_ })
...
function serializeTransactionEIP4844(transaction, signature) {
	...
	if (
		transaction.blobs &&
		(typeof blobVersionedHashes === 'undefined' || typeof sidecars === 'undefined')
	) {
		const blobs2 =
			typeof transaction.blobs[0] === 'string'
				? transaction.blobs
				: transaction.blobs.map((x) => bytesToHex3(x))
...
function serializeTransactionEIP1559(transaction, signature) {
	const { chainId, gas, nonce, to, value, maxFeePerGas, maxPriorityFeePerGas, accessList, data } =
		transaction
	assertTransactionEIP1559(transaction)
	const serializedAccessList = serializeAccessList(accessList)
	const serializedTransaction = [
...
function serializeTransactionLegacy(transaction, signature) {
	...
	if (signature) {
		const v = (() => {
			...
			if (signature.v !== v2) throw new InvalidLegacyVError({ v: signature.v })
			return v2
		})()
		...
		serializedTransaction = [...serializedTransaction, numberToHex(chainId), '0x', '0x']
	}
					...
					async () => {
						try {
							...
							switch (err.code) {
								...
								default:
									if (err_ instanceof BaseError2) throw err_
...
function serializeTypedData(parameters) {
	...
	const message = (() => {
		if (primaryType === 'EIP712Domain') return
...
function from3(value) {
	const publicKey = (() => {
		...
		if (typeof x === 'bigint' && typeof y === 'bigint') return { prefix: prefix ?? 4, x, y }
		return { prefix, x }
	})()
	assert3(publicKey)
	return publicKey
...
function checksum3(address) {
	if (checksum2.has(address)) return checksum2.get(address)
...
function from4(address, options = {}) {
	...
	assert4(address)
	if (checksumVal) return checksum3(address)
	return address
}
...
function decodeParameter2(cursor, param, options) {
	...
	if (param.type === 'address') return decodeAddress2(cursor, { checksum: checksumAddress2 })
	if (param.type === 'bool') return decodeBool2(cursor)
	if (param.type.startsWith('bytes')) return decodeBytes2(cursor, param, { staticPosition })
	if (param.type.startsWith('uint') || param.type.startsWith('int'))
		return decodeNumber2(cursor, param)
	if (param.type === 'string') return decodeString2(cursor, { staticPosition })
...
function decodeAddress2(cursor, options = {}) {
	...
	const value = cursor.readBytes(32)
	const wrap = (address) => (checksum4 ? checksum3(address) : address)
...
function decodeNumber2(cursor, param) {
	...
	const value = cursor.readBytes(32)
	return [size5 > 48 ? toBigInt2(value, { signed }) : toNumber2(value, { signed }), 32]
}
...
function prepareParameter({
	checksumAddress: checksumAddress2 = false,
...
function hasDynamicChild2(param) {
	...
	if (type === 'bytes') return true
	if (type.endsWith('[]')) return true
	if (type === 'tuple') return param.components?.some(hasDynamicChild2)
	const arrayComponents = getArrayComponents2(param.type)
	if (
		arrayComponents &&
		hasDynamicChild2({
...
var staticCursor2 = {
	...
	decrementPosition(offset) {
		if (offset < 0) throw new NegativeOffsetError2({ offset })
		const position = this.position - offset
		this.assertPosition(position)
		this.position = position
	},
...
function decode(parameters, data, options = {}) {
	...
	if (size2(bytes) === 0 && parameters.length > 0) throw new ZeroDataError()
	...
	for (let i = 0; i < parameters.length; ++i) {
		...
		consumed += consumed_
		if (as === 'Array') values.push(data2)
		else values[param.name ?? i] = data2
	}
	return values
}
...
;((encodePacked2) => {
	function encode3(type, value, isArray = false) {
		...
		if (type === 'string') return fromString2(value)
...
class BytesSizeMismatchError2 extends BaseError3 {
	constructor({ expectedSize, value }) {
		super(
			`Size of bytes "${value}" (bytes${size3(value)}) does not match expected size (bytes${expectedSize}).`,
		)
		Object.defineProperty(this, 'name', {
			enumerable: true,
			configurable: true,
...
function from6(value, options) {
	...
	encodable.encode(cursor)
	if (as === 'Hex') return fromBytes(cursor.bytes)
	return cursor.bytes
}
...
function getEncodable2(bytes) {
	if (Array.isArray(bytes)) return getEncodableList2(bytes.map((x) => getEncodable2(x)))
...
function assert5(signature, options = {}) {
	...
	if (signature.r < 0n || signature.r > maxUint2562) throw new InvalidRError({ value: signature.r })
	if (signature.s < 0n || signature.s > maxUint2562) throw new InvalidSError({ value: signature.s })
...
function fromHex4(signature) {
	...
	const yParity = (() => {
		...
		if (Number.isNaN(yParity2)) return
		try {
...
function extract2(value) {
	if (typeof value.r === 'undefined') return
	if (typeof value.s === 'undefined') return
	return from7(value)
}
function from7(signature) {
	const signature_ = (() => {
		if (typeof signature === 'string') return fromHex4(signature)
		if (signature instanceof Uint8Array) return fromBytes3(signature)
		if (typeof signature.r === 'string') return fromRpc2(signature)
...
function fromRpc2(signature) {
	const yParity = (() => {
		...
		if (typeof v === 'number' && typeof yParity2 !== 'number') yParity2 = vToYParity(v)
		if (typeof yParity2 !== 'number') throw new InvalidYParityError({ value: signature.yParity })
		return yParity2
	})()
	return {
...
function from8(authorization, options = {}) {
	if (typeof authorization.chainId === 'string') return fromRpc3(authorization)
...
function assert6(value) {
	if (typeof value === 'string') {
		if (slice3(value, -32) !== magicBytes) throw new InvalidWrappedSignatureError(value)
	} else assert5(value.authorization)
}
...
async function getTransaction(
	client,
	{ blockHash, blockNumber, blockTag: blockTag_, hash: hash3, index: index2, sender, nonce },
) {
	const blockTag = blockTag_ || 'latest'
	const blockNumberHex = blockNumber !== undefined ? numberToHex(blockNumber) : undefined
...
async function getTransactionReceipt(client, { hash: hash3 }) {
	...
	if (!receipt) throw new TransactionReceiptNotFoundError({ hash: hash3 })
...
async function multicall(client, parameters) {
	...
	const { batchSize = parameters.batchSize ?? 1024, deployless = parameters.deployless ?? false } =
		typeof client.batch?.multicall === 'object' ? client.batch.multicall : {}
	const multicallAddress = (() => {
	...
	if (results.length !== contracts.length) throw new BaseError2('multicall results mismatch')
	return results
}
...
async function simulateBlocks(client, parameters) {
	...
	try {
		...
		const result = await client.request({
			...
			params: [{ blockStateCalls, returnFullTransactions, traceTransfers, validation }, block],
		})
		return result.map((block2, i) => ({
			...
			calls: block2.calls.map((call2, j) => {
				...
				const error = (() => {
					...
					let error2
					if (data === '0x') error2 = new AbiDecodingZeroDataError()
					else if (data) error2 = new RawContractError({ data })
					if (!error2) return
...
function normalizeSignature2(signature) {
	...
	for (let i = 0; i < signature.length; i++) {
		...
		if (!active) continue
		if (level === 0) {
			if (char === ' ' && ['event', 'function', 'error', ''].includes(result)) result = ''
			else {
				result += char
				if (char === ')') {
					valid = true
	...
	if (!valid) throw new BaseError3('Unable to normalize signature.')
	return result
}
...
function fromAbi(abi2, name, options) {
	...
	const abiItems = abi2.filter((abiItem2) => {
		if (isSelector) {
			...
			if (abiItem2.type === 'event') return getSignatureHash(abiItem2) === name
			return false
		}
	...
	for (const abiItem2 of abiItems) {
		...
		if (!abiItem2.inputs) continue
		if (abiItem2.inputs.length === 0) continue
		if (abiItem2.inputs.length !== args.length) continue
...
function encode3(...parameters) {
	...
	return concat2(
		bytecode,
		abiConstructor.inputs?.length && args?.length ? encode2(abiConstructor.inputs, args) : '0x',
	)
}
...
function fromAbi3(abi2, name, options) {
	...
	if (item.type !== 'function') throw new NotFoundError({ name, type: 'function' })
	return item
}
...
async function simulateCalls(client, parameters) {
	...
	const assetAddresses = traceAssetChanges
		? await Promise.all(
				parameters.calls.map(async (call2) => {
					if (!call2.data && !call2.abi) return
				...
				? [
						...
						},
						{
							calls: assetAddresses.map((address, i) => ({
								abi: [from12('function balanceOf(address) returns (uint256)')],
			...
			...(traceAssetChanges
				? [
						...
						},
						{
							calls: assetAddresses.map((address, i) => ({
								to: address,
								abi: [from12('function decimals() returns (uint256)')],
						...
						},
						{
							calls: assetAddresses.map((address, i) => ({
								to: address,
								abi: [from12('function tokenURI(uint256) returns (string)')],
								functionName: 'tokenURI',
								args: [0n],
								from: zeroAddress,
								nonce: i,
...
function assert7(wrapped) {
	if (slice3(wrapped, -32) !== magicBytes2) throw new InvalidWrappedSignatureError2(wrapped)
}
function from13(wrapped) {
	if (typeof wrapped === 'string') return unwrap2(wrapped)
	return wrapped
}
...
function wrap2(value) {
	...
	return concat2(encode2(from5('address, bytes, bytes'), [to, data, signature]), magicBytes2)
}
...
async function verifyHash(client, parameters) {
	const {
		...
		chain: chain2 = client.chain,
		hash: hash3,
		erc6492VerifierAddress: verifierAddress = parameters.universalSignatureVerifierAddress ??
			chain2?.contracts?.erc6492Verifier?.address,
		multicallAddress = parameters.multicallAddress ?? chain2?.contracts?.multicall3?.address,
	} = parameters
	if (chain2?.verifyHash) return await chain2.verifyHash(client, parameters)
	const signature = (() => {
		const signature2 = parameters.signature
		if (isHex(signature2)) return signature2
		if (typeof signature2 === 'object' && 'r' in signature2 && 's' in signature2)
			return serializeSignature(signature2)
		return bytesToHex3(signature2)
	})()
	try {
...
async function verifyErc6492(client, parameters) {
	...
	const wrappedSignature = await (async () => {
		...
		if (exports_SignatureErc6492.validate(signature)) return signature
			...
			async onBlockNumber(blockNumber_) {
				...
				try {
					if (receipt) {
						if (
							confirmations > 1 &&
							(!receipt.blockNumber || blockNumber - receipt.blockNumber + 1n < confirmations)
						)
							return
					...
					if (
						...
						try {
							...
							const replacementTransaction = block.transactions.find(
								({ from: from14, nonce }) =>
									from14 === replacedTransaction.from && nonce === replacedTransaction.nonce,
							)
...
function watchPendingTransactions(
	client,
	{ batch = true, onError, onTransactions, poll: poll_, pollingInterval = client.pollingInterval },
) {
	const enablePolling =
			...
			const unwatch = poll(
				async () => {
					try {
						...
						else for (const hash3 of hashes) emit.onTransactions([hash3])
					} catch (err) {
						emit.onError?.(err)
					}
				},
...
async function prepareAuthorization(client, parameters) {
	...
	if (typeof authorization.chainId === 'undefined')
		authorization.chainId =
			client.chain?.id ?? (await getAction(client, getChainId, 'getChainId')({}))
...
async function sendCallsSync(client, parameters) {
	...
	const timeout = parameters.timeout ?? Math.max((chain2?.blockTime ?? 0) * 3, 5000)
...
function createTransport(
	{ key, methods, name, request, retryCount = 3, retryDelay = 150, timeout, type },
	value,
) {
	const uid2 = uid()
	return {
...
async function sign({ hash: hash3, privateKey, to = 'object' }) {
	...
	return (() => {
		if (to === 'bytes' || to === 'hex') return serializeSignature({ ...signature, to })
		return signature
	})()
}
...
function assertTransactionDeposit(transaction) {
	...
	if (from14 && !isAddress(from14)) throw new InvalidAddressError({ address: from14 })
	if (to && !isAddress(to)) throw new InvalidAddressError({ address: to })
}
...
var RPC_URL = process.env.MOR_RPC_URL || 'https://mainnet.base.org'
var CONTRACTS = {
...
function getDerivationPath(index2) {
	return `m/44'/60'/0'/0/${index2}`
}
function generateNewMnemonic(strength = 128) {
	return generateMnemonic(wordlist, strength)
}
function isValidMnemonic(mnemonic) {
	return validateMnemonic(mnemonic, wordlist)
}
function deriveWalletFromMnemonic(mnemonic, index2 = 0) {
...
function getPrivateKey(config) {
	...
	if (envMnemonic) {
		...
		const wallet = deriveWalletFromMnemonic(envMnemonic, index2)
		return wallet.privateKey
	}
...
function getWalletClient(privateKey, rpcUrl) {
	const account = privateKeyToAccount(privateKey)
...
async function swapEthForMor(config, ethAmount) {
	const privateKey = getPrivateKey(config)
	const account = getAccount(privateKey)
...
async function swapUsdcForMor(config, usdcAmount) {
	const privateKey = getPrivateKey(config)
	const account = getAccount(privateKey)
	const publicClient = getPublicClient(config.rpcUrl)
	const walletClient = getWalletClient(privateKey, config.rpcUrl)
...
async function approveMor(config, amount) {
	const privateKey = getPrivateKey(config)
	const publicClient = getPublicClient(config.rpcUrl)
	const walletClient = getWalletClient(privateKey, config.rpcUrl)
...
class MorDiemSDK {
	...
	constructor(config = {}) {
		...
		if (config.mnemonic) {
			...
			const index2 = config.walletIndex ?? 0
			this.wallet = deriveWalletFromMnemonic(config.mnemonic, index2)
	...
	static generateMnemonic(strength = 128) {
		const { generateMnemonic: generateMnemonic2 } = require_bip39()
		const { wordlist: wordlist2 } = require_english()
		return generateMnemonic2(wordlist2, strength)
	}
	...
	get address() {
		...
		if (this._privateKey) {
			return getAccount(this._privateKey).address
		}
	...
	async getBalances() {
		return getBalances(this.walletConfig)
	}
	async swapEthForMor(amount) {
		return swapEthForMor(this.walletConfig, amount)
	}
	async swapUsdcForMor(amount) {
		return swapUsdcForMor(this.walletConfig, amount)
	}
	async approveMor(amount) {
		return approveMor(this.walletConfig, amount)
	}
	async complete(message, options) {
